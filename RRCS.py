        ...
        def pay_window():
            def insert():
                try:
                    ...
                    sheet["L3"] = dropdown_var.get() + number_entry.get()
                    sheet["K9"] = amount
                    if data[8] == "None":
            ...
            options = ["MAL-", "MKH-", "SHW-", "QM-", "JWT-", "ZWW-", "RAS-"]
            dropdown_var = ctk.StringVar(value=options[0])

            dropdown_label = ctk.CTkLabel(newWindow, text=":واحد")
            dropdown_label.place(x=280, y=130)
